...
interface IOptimisticOracleV2 {
    ...
    /// @param ancillaryData ancillary data representing additional args being passed with the price
    /// request.
    /// @param currency ERC20 token used for payment of rewards and fees. Must be approved for use
    /// with the DVM.
    /// @param reward reward offered to a successful proposer. Will be pulled from the caller. Note:
    /// this can be 0,
    ///               which could make sense if the contract requests and proposes the value in the
    /// same call or
    ///               provides its own reward system.
    /// @return totalBond default bond (final fee) + final fee that the proposer and disputer will
    /// be required to pay.
    ...
    /// @return totalBond the amount that's pulled from the proposer's wallet as a bond. The bond
    /// will be returned to
    ...
    ///
    /// 1. The timestamp at which the request is evaluated is the time of the proposal, not the
    ...
    ///
    /// 3. RefundoOnDispute is automatically set, meaning disputes trigger the reward to be
    ...
    /// @notice Sets a custom liveness value for the request. Liveness is the amount of time a
    /// proposal must wait before
    /// being auto-resolved.
    /// @param identifier price identifier to identify the existing request.
    /// @param timestamp timestamp to identify the existing request.
    /// @param ancillaryData ancillary data of the price being requested.
    ...
    /// is not settled
    /// or settleable. Note: this method is not view so that this call may actually settle the price
    /// request if it
    /// hasn't been settled.
    /// @param identifier price identifier to identify the existing request.
    /// @param timestamp timestamp to identify the existing request.
